...
vi.mock("../../config/redis", () => ({
    redis: { del: vi.fn(), keys: vi.fn().mockResolvedValue([]) },
    cacheAside: async <T>(_k: string, _t: number, fn: () => Promise<T>): Promise<T> => fn(),
    nearbyKey: () => "test-key",
    invalidateNearbyCache: vi.fn(),
}));
...
describe("ListEventsService", () => {
    ...
    it("should call findMany with bounding-box WHERE clause", async () => {
        mockFindMany.mockResolvedValue([]);

        await service.listEvents({ latitude: -23.5505, longitude: -46.6333, radiusKm: 10 });
