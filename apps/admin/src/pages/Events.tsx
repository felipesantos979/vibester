import { AlertTriangle, Calendar } from 'lucide-react';

export default function Events() {
  return (
    <div className="space-y-8 pb-10">
      {/* Aviso de dados reais */}
      <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl px-5 py-3 flex items-start gap-3">
        <AlertTriangle className="w-5 h-5 text-yellow-400 shrink-0 mt-0.5" />
        <div>
          <p className="text-sm text-yellow-300 font-medium">Eventos em construção na API</p>
          <p className="text-xs text-yellow-400/70 mt-1">
            Removemos os eventos falsos (mockados). A gestão de eventos e confirmações de presença (VIP)
            estará disponível aqui assim que a equipe de backend publicar os novos endpoints do serviço de eventos.
          </p>
        </div>
      </div>

      <div>
        <h2 className="text-3xl font-bold text-white tracking-tight">Eventos & Promos</h2>
        <p className="text-gray-400 mt-1">Gerencie a agenda do seu estabelecimento.</p>
      </div>

      <div className="glass-panel rounded-2xl p-12 flex flex-col items-center justify-center opacity-70 border border-dashed border-white/20">
        <Calendar className="w-16 h-16 text-gray-600 mb-4" />
        <h3 className="text-xl font-bold text-gray-400 mb-2">Aguardando Backend</h3>
        <p className="text-sm text-gray-500 text-center max-w-md">
          A funcionalidade de criar eventos, visualizar confirmados e gerenciar listas VIP 
          foi temporariamente desabilitada até que a integração com a API real seja concluída.
        </p>
      </div>
    </div>
  );
}
